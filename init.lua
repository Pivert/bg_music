...
function bg_music.check_player_zone(player)
	...
	if closest_zone then
		...
		if not current_music or current_music.zone ~= closest_zone then
			-- Start new music with random song
			local filtered_songs = bg_music.filter_songs(closest_zone.filter)
			if #filtered_songs > 0 then
				local song_index = math.random(1, #filtered_songs)
				local song_name = filtered_songs[song_index]
...
minetest.register_node(":bg_music:stereo", {
	...
	on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
		...
		-- Check if player is already listening to stereo music
		if bg_music.stereo_music[player_name] then
			-- Stop the music
			if bg_music.stereo_music[player_name].handle then
				minetest.sound_stop(bg_music.stereo_music[player_name].handle)
			end
			bg_music.stereo_music[player_name] = nil
		...
		-- Pick a random song for stereo music
		local song_name = bg_music.available_songs[math.random(1, #bg_music.available_songs)]
		...
		if handle then
			bg_music.stereo_music[player_name] = {
				handle = handle,
				pos = pos,
				is_stereo = true
			}
